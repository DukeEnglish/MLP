...
def fully_connected_layer(inputs, input_dim, output_dim, nonlinearity=tf.nn.elu, dropout = True, keep_prob=1.0):
...
def fully_bn_connected_layer(inputs, input_dim, output_dim, nonlinearity=tf.nn.elu, dropout = True, keep_prob=1.0, train=True):
    with tf.device('/cpu:0'):
        weights = tf.Variable(
            tf.truncated_normal(
                [input_dim, output_dim], stddev=2. / (input_dim + output_dim)**0.5), 
...
def conv_bn_layer1(inputs, input_dim, output_dim, nonlinearity=tf.nn.elu, dropout = True, keep_prob=1.0,train=True):
...
def conv_bn_layer2(inputs, input_dim, output_dim, nonlinearity=tf.nn.elu, dropout = True, keep_prob=1.0,train=True):
 kernel = _variable_with_weight_decay('weights1',
                                      shape=[5, 5, 3, 64],
                                      stddev=5e-2,
                                      wd=0.0001)
...
with tf.variable_scope('conv1') as scope:
    ...
    conv1 = tf.nn.elu(pre_activation1, name=scope.name)
...
with tf.variable_scope('conv2') as scope:
    ...
    conv2 = tf.nn.elu(pre_activation2, name=scope.name)
